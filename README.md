...
**A note on basic-auth users and drift**: the Camunda Helm chart only ever seeds `orchestration.security.initialization.users` (the `venom`/`connectors` basic-auth accounts) when Zeebe's storage is first bootstrapped — a later `helm upgrade` re-renders the ConfigMap but never retroactively updates an already-existing user. If `AUTH_PASS`/`CONNECTORS_PASS` ever change without a full Zeebe re-bootstrap, the real password can silently drift out of sync with what's configured. `promote-region.sh` guards against this on every promotion by calling the Orchestration Cluster REST API's user-update endpoint (`PUT /v2/users/{username}`) to force both users back into sync, the same way it already keeps Identity/Keycloak's `venom` login in sync via the Keycloak Admin API. That guard itself authenticates as `AUTH_USER`, so it can only succeed if that credential is *currently* valid — if `AUTH_USER`'s own password has already drifted, only a full re-bootstrap (`./scripts/reset-cluster.sh`) recovers it.
