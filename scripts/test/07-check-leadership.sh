#!/usr/bin/env bash
# Step 7: Partition leadership check + rebalance
...
# regions afterward.
#
# An explicit rebalance step is necessary specifically because Zeebe is never
# restarted by promote/demote in this suite - there is no other point in the
# failover/failback flow that would naturally nudge leadership back toward
# east, so this step provides that deliberate nudge.
set -euo pipefail
source "$(cd "$(dirname "$0")/.." && pwd)/lib/common.sh"

header "STEP 7: Partition leadership check + rebalance"
