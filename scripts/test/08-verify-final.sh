...
require_state TEST_IDENTITY_USER "./05-create-data-during-outage.sh"
require_state TEST_IDENTITY_ROLE "./05-create-data-during-outage.sh"
...
ACTIVE_KEYS="$TEST_BASELINE_ACTIVE_KEYS $TEST_DURING_ACTIVE_KEYS"
...
ENGINE_KEY=$(echo "$TEST_BASELINE_ACTIVE_KEYS" | awk '{print $1}')
ENGINE_OK=0
...
echo
info "Architecture proof: '$TEST_IDENTITY_USER' + role '$TEST_IDENTITY_ROLE' were created via WEST's Keycloak during the outage (05-create-data-during-outage.sh). Confirming they're visible via EAST's Keycloak now, post-failback - proving both regions genuinely share one realm/database, not two independent ones that merely look alike..."
IDENTITY_OK=0
...
if [ -n "$KC_TOKEN" ]; then
  USER_ID=$(curl -s "http://localhost:18380/auth/admin/realms/camunda-platform/users?username=$TEST_IDENTITY_USER" \
...
print('yes' if any(r.get('name')=='$TEST_IDENTITY_ROLE' for r in d) else 'no')
" 2>/dev/null)
  fi
  printf 'Check\tResult\n' > /tmp/dr_tableD8_identity.tsv
  printf 'User %s found via east\t%s\n' "$TEST_IDENTITY_USER" "$([ -n "$USER_ID" ] && echo yes || echo no)" >> /tmp/dr_tableD8_identity.tsv
  printf 'Role %s assigned\t%s\n' "$TEST_IDENTITY_ROLE" "$ROLE_ASSIGNED" >> /tmp/dr_tableD8_identity.tsv
...
if [ "$EAST_COUNT" -eq "$COUNT" ] && [ "$WEST_COUNT" -eq "$COUNT" ] && [ "$EAST_ACTIVE" -eq "$ACTIVE_COUNT" ] && [ "$WEST_ACTIVE" -eq "$ACTIVE_COUNT" ] && [ "$EAST_PASSIVE_UP" -gt 0 ] && [ "$WEST_PASSIVE_DOWN" -eq 0 ] && [ "$ENGINE_OK" -eq 1 ] && [ "$IDENTITY_OK" -eq 1 ]; then
  ok "PASSED: all $COUNT instances (completed + still-active) present in both regions, and roles correctly restored (east active, west passive)."
  echo
  echo "${C_BOLD}${C_GREEN}Summary: full region loss (Zeebe + Elasticsearch + active-passive components${C_RESET}"
  echo "${C_BOLD}${C_GREEN}together) - the hardest scenario. Recovery required a fresh Zeebe bootstrap to${C_RESET}"
  echo "${C_BOLD}${C_GREEN}restore true RF4, an Elasticsearch snapshot/restore across the outage, plus${C_RESET}"
  echo "${C_BOLD}${C_GREEN}promoting the secondary site and re-promoting the primary once it recovered -${C_RESET}"
  echo "${C_BOLD}${C_GREEN}all without ever wiping the survivor's storage.${C_RESET}"
else
  fail "FAILED: instance counts, active-state, or component roles do not match expectations. See tables above."
  exit 1
fi
